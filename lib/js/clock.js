  ...
  function Clock(props) {
    ...
    _this.handleSetupInput = function (category, change) {
      if (!_this.state.running) {
        ...
        if (change === "increment" && _this.state.times[category] < 60) {
          newTimes[category] = _this.state.times[category] + 1;
        } else if (change === "decrement" && _this.state.times[category] > 1) {
          newTimes[category] = _this.state.times[category] - 1;
        }
    ...
    _this.startClock = function () {
      _this.setState(function (state) {
        return {
          running: true,
          paused: false,
          timerId: window.setTimeout(_this.clockStep, state.timeToNextStep),
    ...
    _this.pauseClock = function () {
      window.clearTimeout(_this.state.timerId);
      var timeLeft = _this.state.startingTime + _this.state.timeToNextStep - Date.now();
      if (timeLeft < 0) {
        timeLeft = 0;
      }
      _this.setState({
    ...
    _this.clockStep = function () {
      if (_this.state.clock.minutes === 0 && _this.state.clock.seconds === 0) {
        ...
        _this.setState({
          startingTime: Date.now(),
          timeToNextStep: 1000,
          timerId: window.setTimeout(_this.clockStep, 1000)
        });
      } else if (_this.state.clock.seconds > 0) {
        _this.setState(function (state) {
          return {
            clock: {
